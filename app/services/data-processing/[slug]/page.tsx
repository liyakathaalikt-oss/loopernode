...
export function generateStaticParams() {
  return dataProcessingServices.map((service) => ({
    slug: service.slug,
  }));
}
...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const service = dataProcessingServices.find((s) => s.slug === slug);
  if (!service) return {};
...
export default async function DataProcessingServicePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const relatedServices = dataProcessingServices
          ...
          <div className="container-custom max-w-7xl mx-auto px-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
              {relatedServices.map((related) => (
                <Link prefetch={false} key={related.slug} href={`/services/data-processing/${related.slug}`} className="group block">
